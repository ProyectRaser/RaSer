...
# ----------------------------------------
# (ELIMINADO) Secret manual de Alertmanager
# ----------------------------------------
# Este bloque ya no se necesita porque ahora Helm se encarga de inyectar la config
...
resource "null_resource" "apply_fastapi_resources" {
  ...
  depends_on = [kubernetes_secret.firebase_key]
}

# ----------------------------------------
#  CAMBIO: HELM INSTALL con values YAML personalizado
# ----------------------------------------
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      helm upgrade --install kube-prometheus prometheus-community/kube-prometheus-stack \
        ...
        --create-namespace \
        -f ${path.module}/helm-values/alertmanager-config.yaml
